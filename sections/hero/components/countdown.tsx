...
export function Countdown({ loaded, days, hours, minutes, dateLabel, labels }: CountdownProps) {
  return (
    ...
    >
      <p className="text-base sm:text-lg md:text-xl tracking-[0.15em] uppercase text-accent font-serif mb-3">
        {dateLabel}
      </p>
      <div className="flex items-end gap-4 sm:gap-5 font-serif text-muted-foreground">
        <span className="flex flex-col items-center">
          <span className="text-xs sm:text-sm md:text-base tabular-nums leading-none">
            {String(days).padStart(2, '0')}
          </span>
          <span className="text-[7px] sm:text-[8px] uppercase tracking-[0.1em] text-accent mt-1">
            {labels.days}
          </span>
        </span>
        <span className="flex flex-col items-center">
